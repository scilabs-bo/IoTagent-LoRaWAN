...
function getAppByDeviceGroup(service, subservice, apikey, resource) {
    ...
    return (
        loraApps
            .filter((app) => app instanceof appService.AbstractAppService)
            .find((app) => {
...
function removeAppByDeviceGroup(service, subservice, apikey, resource) {
    ...
    loraApps = loraApps.filter((app) => {
        if (app instanceof appService.AbstractAppService) {
            const iotaConf = app.getIotaConfiguration();
            return !(
                iotaConf.service === service &&
                iotaConf.subservice === subservice &&
...
async function registerApplicationServer(appServerConf, iotaConfiguration) {
    ...
    try {
        ...
        for (const app of loraApps) {
            if (app instanceof appService.AbstractAppService) {
                if (app.getAppId() === appServerConf.lorawan.app_eui) {
                    winston.info('LoRaWAN Application exists');
                    if (iotaConfiguration && app.getIotaConfiguration()) {
...
async function stopApplicationServers() {
    await loraAppsLock.acquireAsync();
    try {
        const loraAppsStopPromises = loraApps
            .filter((app) => app instanceof appService.AbstractAppService)
            .map((app) => {
...
async function registerConfiguration(configuration) {
    ...
    try {
        ...
        if (!ttnApp) {
            throw 'error creating application server';
        }
...
async function removeConfiguration(configuration) {
    ...
    await loraAppsLock.acquireAsync();
    try {
        const appIndex = loraApps.findIndex(
            (loraApp) =>
                loraApp instanceof appService.AbstractAppService && loraApp.getAppId() === lorawanConf.lorawan.app_eui
        );
        if (appIndex >= 0) {
...
async function loadTypesFromConfig() {
    async function registerConfigType(type) {
        if (type) {
    ...
    if (config.getConfig().iota.types) {
        const registerConfigTypesPromises = Object.keys(config.getConfig().iota.types)
            .filter(
                (type) =>
            ...
            .map((type) => registerConfigType(type));

        try {
            await Promise.all(registerConfigTypesPromises);
        } catch (e) {
...
async function loadServices() {
    ...
    if (services && Array.isArray(services.services)) {
        try {
            await Promise.all(services.services.map((service) => registerConfiguration(service)));
        } catch (e) {
...
async function loadDevices() {
    ...
    if (devices && Array.isArray(devices.devices)) {
        try {
            await Promise.all(devices.devices.map((device) => registerDevice(device)));
        } catch (e) {
