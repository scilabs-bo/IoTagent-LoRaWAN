...
function loadTypesFromConfig(callback) {
    function registerConfigType(type, cb) {
        if (type) {
            registerConfiguration(type, cb);
        } else {
            return cb(null);
        }
    }
...
function loadServices(callback) {
    ...
    confService.list(null, 100, 0, function(err, services) {
        ...
        if (services.count > 0 && services.services.length > 0) {
            async.eachSeries(services.services, registerConfiguration, function(e) {
                if (e) {
...
function loadDevices(callback) {
    ...
    iotAgentLib.listDevices(undefined, undefined, undefined, undefined, function(err, devices) {
        ...
        if (devices) {
            async.eachSeries(devices.devices, registerDevice, function(e) {
                if (e) {
...
function start(newConfig, callback) {
    ...
    iotAgentLib.activate(config.getConfig().iota, function(error) {
        ...
        async.waterfall([loadTypesFromConfig, loadServices, loadDevices], function(err) {
            if (err) {
                winston.error(err);
                return callback(err);
            }
