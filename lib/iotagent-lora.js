...
async function registerApplicationServer(appServerConf, iotaConfiguration) {
    ...
    if (
        ...
        ['TTN', 'TTNv3', 'loraserver.io'].indexOf(appServerConf.lorawan.application_server.provider) === -1
    ) {
        error = 'Provider for application server is required. Supported values: TTN, TTNv3 and loraserver.io';
        winston.error(error);
        throw error;
    }
    ...
    try {
        ...
        switch (appServerConf.lorawan.application_server.provider) {
            case 'TTN':
                NewAppProvider = ttnAppService.TtnAppService;
                break;
            case 'TTNv3':
                NewAppProvider = TTNv3AppService;
                break;
            case 'loraserver.io':
                NewAppProvider = loraserverioAppService.LoraserverIoService;
                break;
